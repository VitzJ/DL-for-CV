...
# find mean and std of the image
...
plt.imshow(dataset_from_HF["train"][0]['image'])
plt.axis('off')
plt.show()
...
print(dataset_from_HF["train"][i]['file_name'])
...
resize_transform = transforms.Resize((224, 224)) # resize

samples = []

temp_dir = tempfile.mkdtemp() # temp directory to store resized images
...
for i, frame in enumerate(dataset_from_HF['train']):  # for every image from HF dataset
  ...
  temp_image_path = os.path.join(temp_dir, f"resized_image_{i}.png") # save resized image to a temporary file
  image_resized.save(temp_image_path, format="PNG") # save as PNG

  sample = fo.Sample(filepath=temp_image_path) # use temp file path
  sample["label"] = label # add label
...
fo_HF_dataset.sort_by("label") # sort by label

"""The images will load as long as the code below it is not run. If I load them here, it causes the size of the file to balloon to over 25 mb."""
...
# comment this out to see the images
session.close()
# Manually delete the temporary directory and its contents and close the session
